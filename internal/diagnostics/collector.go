...
func New(options Options) *Collector {
	...
	return &Collector{
		...
		geo: options.Geo, system: system, now: now,
	}
}
...
func (c *Collector) serviceItem(ctx context.Context) (Item, *host.Status) {
	...
	if err != nil {
		item.Level, item.Summary = LevelUnknown, "无法读取 systemd 状态"
		item.Details = []string{err.Error()}
		item.Suggestion = "确认 systemd 与 dae.service 可用，并检查面板服务权限"
		return item, nil
	}
	item.Details = appendNonEmpty(nil,
		...
		fmt.Sprintf("单元文件：%s", status.UnitPath),
	)
	switch status.ActiveState {
	case "active":
		if status.MainPID <= 0 {
			item.Level, item.Summary = LevelWarning, "服务显示运行中，但 MainPID 无效"
			item.Suggestion = "重启 dae，并检查 systemctl status dae"
...
func (c *Collector) geoItem(ctx context.Context) Item {
	...
	if status.Problem != "" {
		...
		item.Suggestion = "按提示修正目录权限或 kdae-panel.service 的 ReadWritePaths"
		return item
	}
...
func (c *Collector) systemItems(snapshot SystemSnapshot, service *host.Status) []Item {
	...
	if service != nil && service.ActiveState == "active" && service.MainPID > 0 {
		...
		bpf.Suggestion = "升级或更换带 BTF 的 Linux 内核后再启动 dae"
	}
...
func (c *Collector) logItem(ctx context.Context, status *host.Status) []Item {
	...
	if err != nil {
		item.Level, item.Summary = LevelUnknown, "无法读取 journald 日志"
		item.Details = []string{err.Error()}
		return []Item{item}
	}
