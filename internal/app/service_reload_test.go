...
func TestSystemdDaeServiceDefersWhenInactive(t *testing.T) {
...
func TestSystemdDaeServiceRejectsUnstableStateAndInvalidPID(t *testing.T) {
	controller := &stubDaeController{}
	for _, status := range []host.Status{
		{ActiveState: "activating", SubState: "start-pre"},
		{ActiveState: "active", SubState: "running", MainPID: 0},
	} {
		service := newSystemdDaeService(controller, controller, controller, &stubHostService{status: status})
		if err := service.Reload(context.Background()); err == nil {
